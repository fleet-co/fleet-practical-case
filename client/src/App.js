...
function App() {
	...
	useEffect(() => {
		...
		const savedTypeFilter = window.localStorage.getItem("fleet_device_type_filter");
		const savedOwnerFilter = window.localStorage.getItem("fleet_device_owner_filter");
	...
	useEffect(() => {
		...
		if (roleFilter) {
			nextEmployees = nextEmployees.filter((employee) => employee.role === roleFilter);
		}
	...
	useEffect(() => {
		...
		if (deviceTypeFilter) {
			nextDevices = nextDevices.filter((device) => device.type === deviceTypeFilter);
		}
	...
	async function fetchEmployees() {
		...
		try {
			...
			setErrors((prev) => [...prev, `Employees fetch failed: ${error.message}`]);
		} finally {
	...
	async function handleDeleteEmployee(employeeId) {
		const isConfirmed = window.confirm("Delete employee and unassign their devices?");
		if (!isConfirmed) {
			return;
		}
	...
	return (
		<div className="app-page">
			...
			{lastRefreshAt ? <p className="timestamp">Last refresh: {lastRefreshAt}</p> : null}
